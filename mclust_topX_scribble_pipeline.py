...
meta_data_index = ['test_name', 'seed', 'dataset', 'sample', 'n_pcs', 'scribble', 'max_iter', 'sim', 'miu', 'niu','alpha','beta', 'scheme', 'lr', 'nConv', 'no_of_scribble_layers', 'intermediate_channels', 'added_layers', 'last_layer_channel_count', 'hyper_sum_division']
...
for model in tqdm(models):
    ...
    leaf_output_folder_path = f'{output_folder_path}/{scheme}/{n_pcs}_pcs/Seed_{seed}/Lr_{lr}/Hyper_{alpha}_{beta}'
    ...
    df_final_metrics = pd.DataFrame({'ARI': calc_ari(df_man, df_labels), 'Loss': df_loss_per_itr['Loss'].values[-1:], 'Loss_without_hyperparam': df_loss_without_hyperparam_per_itr['Loss_without_hyperparam'].values[-1:]})
    ...
    meta_data_value = [test_name, seed, dataset, sample, n_pcs, scribble, max_iter, stepsize_sim, stepsize_con, stepsize_scr,alpha,beta, scheme, lr, nConv, no_of_scribble_layers, intermediate_channels, added_layers, last_layer_channel_count, hyper_sum_division]
    ...
    plt.savefig(f'{leaf_output_folder_path}/seg_{alpha}_{beta}.png',format='png',dpi=1200,bbox_inches='tight',pad_inches=0)
    plt.savefig(f'{leaf_output_folder_path}/seg_{alpha}_{beta}.eps',format='eps',dpi=1200,bbox_inches='tight',pad_inches=0)
